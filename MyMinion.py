...
class MyMinion(Minion):
    def __init__(self, position, orientation, world, image = NPC, speed = SPEED, viewangle = 360, hitpoints = HITPOINTS, firerate = FIRERATE, bulletclass = SmallBullet):
        ...
        self.states = [Idle,Move,Attack,Spread]
...
class Attack(State):
    ...
    def execute(self, delta = 0):
        ...
        team=self.agent.getTeam()
        aloc=self.agent.getLocation()
        minions=self.agent.world.getNPCsForTeam(team)
        for x in minions:
            if x.getLocation==aloc:
                p=sorted(self.agent.getPossibleDestinations(),key=lambda x:distance(x,aloc))[0]
                
                self.agent.changgeState(Spread,p,self.victim)
...
class Spread(State):
    ...
    def execute(self, delta = 2):
        #print("move")
        aloc=self.agent.getLocation()
        if distance(aloc,self.victim.getLocation())>self.agent.range:
            self.agent.changeState(Idle)
